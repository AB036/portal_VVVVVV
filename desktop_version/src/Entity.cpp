...
void entityclass::init()
{
    ...
    blue_portal_index = -1;
    orange_portal_index = -1;
    blue_portal_orientation = '_';
    orange_portal_orientation = '_';
    player_portal_teleport_cooldown = 0;
}
...
void entityclass::createentity( Game& game, float xp, float yp, int t, float vx /*= 0*/, float vy /*= 0*/, int p1 /*= 0*/, int p2 /*= 0*/, int p3 /*= 320*/, int p4 /*= 240 */ )
{
    ...
    //Rule 1000 is blue portal
    //Rule 1001 is orange portal
    ...
    case 51: //Vertical Warp Line
      ...
      case 56: //Custom enemy
        ...
        break;
      case 1000: //blue portal
        entities[k].rule = 1000;
        entities[k].size = -1;
        entities[k].colour = 1000;
        entities[k].xp = xp;
        entities[k].yp = yp;
        entities[k].w = 24;
        entities[k].h = 24;
        entities[k].dir = 1; //???
        if (blue_portal_index >= 0)
        {
            entities[blue_portal_index].active = false; //deactivate old portal
        }
        blue_portal_index = k;
        break;
      case 1001: //orange portal
        entities[k].rule = 1001;
        entities[k].tile = 0; //???
        entities[k].size = -1;
        entities[k].colour = 1001;
        entities[k].xp = xp;
        entities[k].yp = yp;
        entities[k].w = 24;
        entities[k].h = 24;
        entities[k].dir = 1; //???
        if (orange_portal_index >= 0)
        {
            entities[orange_portal_index].active = false;
        }
        orange_portal_index = k;
        break;
    }
}
...
void entityclass::animateentities( int _i, Game& game, UtilityClass& help )
{
    if(entities[_i].active)
    {
        if(entities[_i].statedelay < 1)
        {
            ...
            case 1000: // blue portal
            case 1001: // orange portal
                entities[_i].drawframe = (entities[_i].drawframe + 8) % 8;
                break;
            default:
                entities[_i].drawframe = entities[_i].tile;
                break;
...
bool portal_collide(char portal_orientation, const entclass& player, const entclass& portal)
{
    int x = player.xp;
    int y = player.yp;
    int vx = player.vx;
    ...
    switch (portal_orientation)
    {
    case 'u':
        if (vy < 0) return false;
        if (!(px - 12 <= x && x <= px + 12)) return false;
        return (py - 14 <= y && y <= py - 10);
    case 'd':
        if (vy > 0) return false;
        if (!(px - 12 <= x && x <= px + 12)) return false;
        return (py + 6 <= y && y <= py + 10);
    case 'l':
        if (vx < 0) return false;
        if (!(py - 5 <= y && y <= py + 7)) return false;
        return (px - 7 <= x && x <= px - 4);
    case 'r':
        if (vx > 0) return false;
        if (!(py - 5 <= y && y <= py + 7)) return false;
        return (px + 2 <= x && x <= px + 7);
    default:
        return false;
    }
}

bool portal_free_space(char portal_orientation, mapclass& map, const entclass& portal)
{
    // build a rectangle in front of the portal and check if there's any wall inside it
    int x1,x2,y1,y2;
    switch (portal_orientation)
    {
    case 'u':
...
void teleport_to_portal(char entry_orientation, char exit_orientation, entclass& player, const entclass& entry_portal, const entclass& exit_portal)
{
    int u = 0;  // position offset at portal entry perpendicular to portal direction

    switch (entry_orientation)
    {
        case 'u':
        case 'd':
            u = player.xp - entry_portal.xp;
            break;
        case 'l':
        case 'r':
...
{
    if (player_portal_teleport_cooldown < 0) player_portal_teleport_cooldown += 1;
    if (player_portal_teleport_cooldown > 0) player_portal_teleport_cooldown -= 1;
    ...
    *ymax = 240;
    for (int i = 0; i < nentity; i++)
    {
        if (entities[i].active && !entities[i].invis)
        {
            ...
            {
                x1 = entities[i].xp + entities[i].cx;
                y1 = entities[i].yp + entities[i].cy;
                x2 = x1 + entities[i].w;
                y2 = y1 + entities[i].h;
            }
            else
            ...
            bool corner_11 = (x1 - xa)*(-yb + ya) + (y1 - ya)*(xb - xa) > 0;
            bool corner_12 = (x1 - xa)*(-yb + ya) + (y2 - ya)*(xb - xa) > 0;
            bool corner_21 = (x2 - xa)*(-yb + ya) + (y1 - ya)*(xb - xa) > 0;
            bool corner_22 = (x2 - xa)*(-yb + ya) + (y2 - ya)*(xb - xa) > 0;

            if (corner_11 != corner_12) // right side of the box is colliding with the line
            {
                if (x1 >= xa)
            ...
            if (corner_21 != corner_22) // left side of the box is colliding with the line
            {
                if (x2 >= xa)
            ...
            if (corner_11 != corner_21) // top side of the box is colliding with the line
            {
                if (y1 >= ya)
            ...
            if (corner_12 != corner_22) // bottom side of the box is colliding with the line
            {
                if (y2 >= ya)
