...
void Game::init(void)
{
    ...
    leftclick = false;
    rightclick = false;
    leftclick_rising_edge = false;
    rightclick_rising_edge = false;
...
void Game::updatestate( Graphics& dwgfx, mapclass& map, entityclass& obj, UtilityClass& help, musicclass& music )
{
    ...
    if (statedelay <= 0)
    {
        ...
        case 23: // PORTAL TEXT BOX
            if (1)//obj.flags[3] == 0)
            {
                dwgfx.textboxremovefast();
                //obj.changeflag(3, 1);
                state = 0;
                dwgfx.createtextbox("  Use the MOUSE to create PORTALS  ", -1, 25, 174, 174, 174);
                dwgfx.textboxtimer(60);
            }
            obj.removetrigger(23);
            break;
