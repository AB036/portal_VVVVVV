...
void titleinput(KeyPoll& key, Graphics& dwgfx, mapclass& map, Game& game, entityclass& obj, UtilityClass& help, musicclass& music)
{
    ...
    {
		//GAMEPAD TODO
        if (key.isDown(KEYBOARD_LEFT) || key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_a) || key.isDown(KEYBOARD_q) ||  key.isDown(KEYBOARD_s) || key.controllerWantsRight(true)) game.press_left = true;
    ...
    {
        if (key.isDown(KEYBOARD_LEFT) || key.isDown(KEYBOARD_UP) || key.isDown(KEYBOARD_a) || key.isDown(KEYBOARD_q) ||  key.isDown(KEYBOARD_w) || key.controllerWantsLeft(true))
        {
               ...
               entityclass& obj, UtilityClass& help, musicclass& music)
{
    ...
    {
        game.leftclick_rising_edge = true;
        key.leftbutton_rising_edge = false;
    }
    else
    {
        game.leftclick_rising_edge = false;
    }
    if (key.rightbutton_rising_edge)
    {
        game.rightclick_rising_edge = true;
        key.rightbutton_rising_edge = false;
    }
    else
    {
        game.rightclick_rising_edge = false;
    }
...
/*
    ...
    { */
        if(!script.running)
        {
            if (key.isDown(KEYBOARD_LEFT) || key.isDown(KEYBOARD_a) || key.isDown(KEYBOARD_q) || key.controllerWantsLeft(false))
            {
                game.press_left = true;
            }
              ...
              entityclass& obj, UtilityClass& help, musicclass& music)
{
    ...
    if(dwgfx.menuoffset==0)
    {
        if (dwgfx.flipmode)
        {
            if (key.isDown(KEYBOARD_LEFT) || key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_a) || key.isDown(KEYBOARD_q) || key.isDown(KEYBOARD_s) || key.controllerWantsLeft(false) ) game.press_left = true;
        ...
        {
            if (key.isDown(KEYBOARD_LEFT) || key.isDown(KEYBOARD_UP) || key.isDown(KEYBOARD_a) || key.isDown(KEYBOARD_q) ||  key.isDown(KEYBOARD_w)|| key.controllerWantsLeft(false))
            {
                game.press_left = true;
            }
                     ...
                     entityclass& obj, UtilityClass& help, musicclass& music)
{
    ...
    if(dwgfx.menuoffset==0)
    {
        if (key.isDown(KEYBOARD_LEFT)|| key.isDown(KEYBOARD_a) || key.isDown(KEYBOARD_q) || key.controllerWantsLeft(false) ) game.press_left = true;
