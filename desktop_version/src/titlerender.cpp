...
void gamerender(Graphics& dwgfx, mapclass& map, Game& game, entityclass& obj, UtilityClass& help, musicclass& music)
{
    ...
    if(!game.blackout)
    {
        ...
        if(!game.completestop)
        {
            for (int i = 0; i < obj.nentity; i++)
            {
                ...
                {
                    px = obj.entities[i].xp;
                    py = obj.entities[i].yp;
                }
        ...
        if (game.leftclick_rising_edge && dwgfx.blue_fire_line_step == 0 && px != -1 && py != -1)
        {
            dwgfx.blue_fire_line_step = 1;
            dwgfx.blue_x_source = px + 12;
            dwgfx.blue_y_source = py + 12;
            dwgfx.blue_x_aim = game.mx;
            dwgfx.blue_y_aim = game.my;
        }
        if (game.rightclick_rising_edge && dwgfx.orange_fire_line_step == 0 && px != -1 && py != -1)
        {
            dwgfx.orange_fire_line_step = 1;
            dwgfx.orange_x_source = px + 12;
            dwgfx.orange_y_source = py + 12;
            dwgfx.orange_x_aim = game.mx;
            dwgfx.orange_y_aim = game.my;
        }
        if (1 <= dwgfx.blue_fire_line_step)
        {
            dwgfx.blue_fire_line_step = (dwgfx.blue_fire_line_step + 1) % 4; // fire line animation length

            //https://en.wikipedia.org/wiki/Bresenham's_line_algorithm
            int x0 = dwgfx.blue_x_source;
            int y0 = dwgfx.blue_y_source;
            int dx = abs(x0 - dwgfx.blue_x_aim);
            int dy = -abs(y0 - dwgfx.blue_y_aim);
            int sx = (x0 < dwgfx.blue_x_aim) ? 1 : -1;
            int sy = (y0 < dwgfx.blue_y_aim) ? 1 : -1;
            int err = dx + dy;
            int e2 = 2*err;
            int pixel_count = 0;
            ...
            if (map.collide(x0/8, y0/8) && dwgfx.blue_fire_line_step == 2) // the fire line hits a wall
            {
                //find orientation of the wall
                char orientation = '_';
                if ( (x0/8) == ((x0-sx)/8) )
                    orientation = (sy == 1) ? 'u' : 'd';
                else if ( (y0/8) == ((y0-sy)/8) )
                    orientation = (sx == 1) ? 'l' : 'r';
                else // diagonal case
                {
                    x0 -= sx;
                    y0 -= sy;
                    int collision = (map.collide((x0+1)/8, y0/8) << 3) // right
                                  + (map.collide((x0-1)/8, y0/8) << 2) // left
                                  + (map.collide(x0/8, (y0+1)/8) << 1) // down
                                  + (map.collide(x0/8, (y0-1)/8) << 0);// up
                    switch (collision)
                    {
                        case 0b1000: orientation = 'l'; x0 += 1; break;
                        case 0b0100: orientation = 'r'; x0 -= 1; break;
                        case 0b0010: orientation = 'u'; y0 += 1; break;
                        case 0b0001: orientation = 'd'; y0 -= 1; break;
                        case 0b1010: orientation = 'u'; y0 += 1; break;
                        case 0b1001: orientation = 'd'; y0 -= 1; break;
                        case 0b0110: orientation = 'u'; y0 += 1; break;
                        case 0b0101: orientation = 'd'; y0 -= 1; break;
                        case 0b0000: orientation = (sy == 1) ? 'u' : 'd'; x0 += sx; y0 += sy; break;
                    }
                }
                bool room_for_portal = false;
                if (map.portal_free(x0/8, y0/8, orientation))
                    room_for_portal = true;
                else if ((orientation == 'u' || orientation == 'd') && map.portal_free(x0/8 - 1, y0/8, orientation))
                {
                    x0 -= 8;
                    room_for_portal = true;
                }
                ...
                else if ((orientation == 'r' || orientation == 'l') && map.portal_free(x0/8, y0/8 - 1, orientation))
                {
                    y0 -= 8;
                    room_for_portal = true;
                }
                ...
                if (room_for_portal)
                {
                    for (int k = -7; k < 16; k++)
                    {
                        switch (orientation)
                        {
                            case 'u': dwgfx.drawpixel(8*(x0/8) + k, 8*(y0/8), 220, 220, 220); break;
                            case 'd': dwgfx.drawpixel(8*(x0/8) + k, 8*(y0/8) + 7, 220, 220, 220); break;
                            case 'l': dwgfx.drawpixel(8*(x0/8), 8*(y0/8) + k, 220, 220, 220); break;
                            case 'r': dwgfx.drawpixel(8*(x0/8) + 7, 8*(y0/8) + k, 220, 220, 220); break;
                        }
                    }
                    obj.blue_portal_orientation = orientation;
                    switch (orientation)
                    {
                        case 'u':
                            obj.createentity(game, 8*(x0/8) - 8, 8*(y0/8) - 11, 1000, 0, 0);
                            obj.entities[obj.blue_portal_index].drawframe = 0;
                            break;
                        case 'd':
                            obj.createentity(game, 8*(x0/8) - 8, 8*(y0/8) - 4, 1000, 0, 0);
                            obj.entities[obj.blue_portal_index].drawframe = 1;
                            break;
                        case 'l':
                            obj.createentity(game, 8*(x0/8) - 12, 8*(y0/8) - 8, 1000, 0, 0);
                            obj.entities[obj.blue_portal_index].drawframe = 2;
                            break;
                        case 'r':
                            obj.createentity(game, 8*(x0/8) - 4, 8*(y0/8) - 8, 1000, 0, 0);
                            obj.entities[obj.blue_portal_index].drawframe = 3;
                            break;
                    }
                    music.playef(12,10);
                }
                ...
                {
                    // could not place a portal
                    music.playef(6,10);
                }
            }
        }
        if (1 <= dwgfx.orange_fire_line_step)
        {
            dwgfx.orange_fire_line_step = (dwgfx.orange_fire_line_step + 1) % 4; // fire line animation length

            //https://en.wikipedia.org/wiki/Bresenham's_line_algorithm
            int x0 = dwgfx.orange_x_source;
            int y0 = dwgfx.orange_y_source;
            int dx = abs(x0 - dwgfx.orange_x_aim);
            int dy = -abs(y0 - dwgfx.orange_y_aim);
            int sx = (x0 < dwgfx.orange_x_aim) ? 1 : -1;
            int sy = (y0 < dwgfx.orange_y_aim) ? 1 : -1;
            int err = dx + dy;
            int e2 = 2*err;
            int pixel_count = 0;
            ...
            obj.line_collision_bounds(x0, y0, dwgfx.orange_x_aim, dwgfx.orange_y_aim, &xmin, &ymin, &xmax, &ymax);
            ...
            if (map.collide(x0/8, y0/8) && dwgfx.orange_fire_line_step == 2) // the fire line hits a wall
            {
                //find orientation of the wall
                char orientation = '_';
                if ( (x0/8) == ((x0-sx)/8) )
                ...
                if (room_for_portal)
                {
                    ...
                    obj.orange_portal_orientation = orientation;
                    switch (orientation)
                    {
                        case 'u':
                            obj.createentity(game, 8*(x0/8) - 8, 8*(y0/8) - 11, 1001, 0, 0);
                            obj.entities[obj.orange_portal_index].drawframe = 0;
                            break;
                        case 'd':
                            obj.createentity(game, 8*(x0/8) - 8, 8*(y0/8) - 4, 1001, 0, 0);
                            obj.entities[obj.orange_portal_index].drawframe = 1;
                            break;
                        case 'l':
                            obj.createentity(game, 8*(x0/8) - 12, 8*(y0/8) - 8, 1001, 0, 0);
                            obj.entities[obj.orange_portal_index].drawframe = 2;
                            break;
                        case 'r':
                            obj.createentity(game, 8*(x0/8) - 4, 8*(y0/8) - 8, 1001, 0, 0);
                            obj.entities[obj.orange_portal_index].drawframe = 3;
                            break;
                    }
                    music.playef(11,10);
                }
...
void towerrender(Graphics& dwgfx, Game& game, mapclass& map, entityclass& obj, UtilityClass& help)
{
    ...
    if (game.leftclick_rising_edge && dwgfx.blue_fire_line_step == 0 && px != -1 && py != -1)
    {
        ...
        dwgfx.blue_y_source = py + 12;
        dwgfx.blue_x_aim = game.mx;
        dwgfx.blue_y_aim = game.my + (int) map.ypos;
    }
    if (game.rightclick_rising_edge && dwgfx.orange_fire_line_step == 0 && px != -1 && py != -1)
    {
        dwgfx.orange_fire_line_step = 1;
        ...
        dwgfx.orange_y_source = py + 12;
        dwgfx.orange_x_aim = game.mx;
        dwgfx.orange_y_aim = game.my + (int) map.ypos;
    }
    if (1 <= dwgfx.blue_fire_line_step)
    {
        dwgfx.blue_fire_line_step = (dwgfx.blue_fire_line_step + 1) % 4; // fire line animation length
        ...
        while (!map.collide(x0/8, y0/8))
        {
            ...
            if ((pixel_count + dwgfx.blue_fire_line_step) % 12 == 0)
                dwgfx.drawpixel(x0+1, y0-1 - map.ypos, 64, 64, 184);
            if ((pixel_count + dwgfx.blue_fire_line_step) % 13 == 0)
                dwgfx.drawpixel(x0-1, y0+1 - map.ypos, 64, 64, 184);
            if ((pixel_count + dwgfx.blue_fire_line_step) % 11 == 0)
                dwgfx.drawpixel(x0+1, y0+1 - map.ypos, 64, 64, 184);
        ...
        if (map.collide(x0/8, y0/8) && dwgfx.blue_fire_line_step == 2) // the fire line hits a wall
        {
            ...
            if (room_for_portal)
            {
                for (int k = -7; k < 16; k++)
                {
                    switch (orientation)
                    {
                        case 'u': dwgfx.drawpixel(8*(x0/8) + k, 8*((y0-(int)map.ypos)/8), 220, 220, 220); break;
                        case 'd': dwgfx.drawpixel(8*(x0/8) + k, 8*((y0-(int)map.ypos)/8) + 7, 220, 220, 220); break;
                        case 'l': dwgfx.drawpixel(8*(x0/8), 8*((y0-(int)map.ypos)/8) + k, 220, 220, 220); break;
                        case 'r': dwgfx.drawpixel(8*(x0/8) + 7, 8*((y0-(int)map.ypos)/8) + k, 220, 220, 220); break;
                    }
                }
                obj.blue_portal_orientation = orientation;
                switch (orientation)
    ...
    if (1 <= dwgfx.orange_fire_line_step)
    {
        ...
        while (!map.collide(x0/8, y0/8))
        {
            ...
            if ((pixel_count + dwgfx.orange_fire_line_step) % 12 == 0)
                dwgfx.drawpixel(x0+1, y0-1 - map.ypos, 144, 94, 64);
            if ((pixel_count + dwgfx.orange_fire_line_step) % 13 == 0)
                dwgfx.drawpixel(x0-1, y0+1 - map.ypos, 144, 94, 64);
            if ((pixel_count + dwgfx.orange_fire_line_step) % 11 == 0)
                dwgfx.drawpixel(x0+1, y0+1 - map.ypos, 144, 94, 64);
